...
for radius in np.linspace(3.0, 4.0, num=11, endpoint=True):
    for offset in np.linspace(-3.0, -2.0, num=11, endpoint=True):
        ...
        # output mesh
        os.system("openscad -o assn6_meshes/bridge_r_{:d}_o_{:d}.stl assn6.scad".format(int(radius*10), int(offset*10)))
